...
export const caseStudyIntro = {
  ...
  context:
    "India's party and venue booking platform: a CodeIgniter 4 core, React web app, Node services, and Flutter customer and partner apps. I led engineering across all four product boards while staying deep in the code, authoring nearly half of all engineering commits across 10 repositories.",
};
...
export const caseStudyChapters: CaseStudyChapter[] = [
  ...
  },
  {
    ...
    body: [
      ...
      "I hardened customer identity across React, Node and CodeIgniter with shared JWT sessions and signed public IDs, auto-refactored 80+ React API endpoints to runtime environment config, and architected the Flutter super-app's split into secure feature repos with contracts, sandboxes and onboarding runbooks, so external contributors ship safely without touching the core.",
    ],
    stats: [
  ...
  },
  {
    ...
    body: [
      "Sales calls vanished into archives, customer chats queued for humans, resumes piled up unread. I built the company an AI layer. The flagship: an async call-intelligence pipeline that transcribes every recorded call (AssemblyAI), summarizes it with a self-hosted Ollama model, and emails leadership a per-agent digest every morning, production-hardened through a 34-item checklist of retry semantics, locking and backfills.",
      "The pattern repeats across the stack: a WhatsApp concierge bot (Meta Cloud API plus Dify) shipped solo in a single day, later extended to the web as AskWitty chat sharing one AI brain across channels; an AI resume screener productionized in a day; AI review moderation with admin-editable rules; and a cohort-normalized feed ranking algorithm tunable from an admin UI without redeploys.",
    ],
    stats: [
  ...
  },
  {
    ...
    subtitle: "Linux, DevOps & security",
    body: [
      "I built the platform's missing safety layers in deliberate order, administering the production Linux fleet end to end. First, recovery: a MySQL backup platform written entirely solo, with AES-256-GCM encrypted dumps streamed to Cloudflare R2, TOTP MFA, tiered retention, one-click permission-aware restore and audit logging. Then the perimeter: SSH and SFTP moved behind a Pritunl VPN server, iptables rules closing six publicly exposed service ports, Docker containers rebound to localhost, Apache virtual hosts cleaned up with mod_proxy fixes, and internal tools (Passbolt password manager, phpMyAdmin) taken off the public internet with automated certbot DNS-01 SSL renewals through Cloudflare.",
