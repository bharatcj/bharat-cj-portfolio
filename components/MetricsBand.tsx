...
import { CountUp, DepthRise, Stagger } from "./motion";
...
export default function MetricsBand() {
  return (
    <section className="relative overflow-hidden border-y border-white/5 bg-white/[0.015] py-16">
      <Stagger
        staggerDelay={0.07}
        className="mx-auto grid max-w-6xl grid-cols-2 gap-8 px-6 sm:grid-cols-3 lg:grid-cols-6"
      >
        {metrics.map((m) => (
          <DepthRise nested key={m.label}>
